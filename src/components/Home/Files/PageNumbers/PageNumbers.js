...
const PageNumbers = ({ grid, cardsPerPage, currentPage, setCurrentPage, showGrid }) => {
      ...
      if (number === 1 && currentPage === 1) {
        ...
        return (
          <button
            ...
            className={number == currentPage ? "btn btn-outline-secondary btn-active" : "btn btn-outline-secondary"}
            style={{ margin: "1%" }}
            key={number}
            id={number}
            onClick={handleClick}
    ...
    return (
        <>
        {
            showGrid ? (<div style={{ marginTop: "20px" }} className="page-numbers">
                    {renderPrevBtn}
                    {pageDecrementBtn}
                    {renderPageNumbers}
                    {pageIncrementBtn}
